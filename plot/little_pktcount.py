...
file_path = "../output/littleFlowPktSize.out"
packet_data = []
...
interval_count = 10
interval_width = (20 - 0) // interval_count
...
packet_proportions = [count for count in packet_counts]

# Generate the x-axis labels, showing a label every 300 units
xtick_labels = [i * interval_width if i * interval_width % 1 == 0 else '' for i in range(interval_count)]

# Plot the bar chart
plt.bar(range(interval_count), packet_proportions, tick_label=xtick_labels, color = cst.style15.color3)

# Set the axis labels and title
plt.xlabel("Flow Packet Count")
