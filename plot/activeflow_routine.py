...
# Read activeflow data from a specified file
file_path = "../output/activeFlow.out"
activeflow_data = []
with open(file_path, "r") as file:
    for line in file:
        # Convert bytes to MB
        activeflow_mb = int(line.strip())
        activeflow_data.append(activeflow_mb)
        i += 1
        if i >= s_limit:
            break
del activeflow_data[:5]
del activeflow_data[-5:]

# Apply moving average to smooth the data
window_size = 10
smoothed_activeflow_data = moving_average(activeflow_data, window_size)

for i in range(ts_limit):
    if i%slice_ts == 0:
        data_group.append([])
    data_group[i//slice_ts].append(smoothed_activeflow_data[i])
